...
export const SearchbarComponent = (props) => {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchKeyword) {
        get_suggestions(searchKeyword, userToken)
          .then((r) => r.json())
          .then((res) => {
            var tempArray = []
  ...
  function openQuestion(ques, i) {
    var searchedItem = document.getElementById('searchInput').value
    setRouteUrl(
      '/' + ques['href'] + '/?searchedTextUrl=' + encodeURI(searchedItem)
    )
    clearSearch()
  }
  ...
  function getSearchHistory(limit) {
    return fetch(
      API_ENDPOINT + 'questions_by_user/' + userToken + '/' + limit + '/',
      {
        method: 'GET',
        headers: {
